...
var dntFileUpload = new FileUpload(
  "drop_zone",
  "files",
  "fileForm",
  "list",
...
var translationFileUpload = new FileUpload(
  "tdrop_zone",
  "tfiles",
  "tFileForm",
  "tlist",
  ...
  if (paramString == null || paramString.length < 2) {
    ...
    if (file == null || file.length < 5) {
      ...
      if (autoTranslate.checked) {
        ...
        dntReader.loadDntFromServerFile(
          location + "/" + file,
          updateProgress,
          refreshTable,
          updateProgress,
...
function dntPageInit() {
  if (window.File && window.FileReader && window.FileList && window.Blob) {
    ...
    alert("Файловый API не полностью поддерживается вашим браузером.")
  }
...
function processFile(data, fileName) {
  try {
    ...
    progress.textContent = "Произошла ошибка во время зарузки"
  }
}

// function to refresh the table
function refreshTable() {
  progress.textContent = "Происходит сброс..."
  ...
  document.title =
    dntReader.fileName
      ...
      .replace("http://", "")
      .replace(".lzjson", "")
      .replace(".firebaseapp.com/", " ") + " (dntviewer)"
  var resultHtml =
    "<h2>" +
    dntReader.fileName +
    " <small>" +
    dntReader.numRows +
    " строк" +
    getNumberPronounce(dntReader.numRows, "а", "и", "") +
    " всего</small> "
...
function getInput(name, index, existinginputs) {
  ...
  return (
    '<input id="' + name + index + '" class="form-control" value="' + value + '" type="text" />'
  )
}
