function FileUpload(
  ...
  this.handleFileDrop = function (evt) {
    ...
    if (files == null || files.length == 0) {
      ...
      this.progress.textContent = "Нет файла"
      return
    }
  ...
  this.loadFile = function (addedFile) {
    ...
    if (!this.isBinary) {
      if (
        fileName.toUpperCase().lastIndexOf(".LZJSON") != fileName.length - 7 &&
        fileName.toUpperCase().lastIndexOf(".XML") != fileName.length - 4
      ) {
        this.progress.textContent = "Некорректный формат файла!"
        dropZone.className = "bg-danger"
        return
      }
    }
    ...
    reader.onabort = function (e) {
      alert("Чтение файла было прервано")
    }
    reader.onloadstart = function (e) {
      document.getElementById(progressDivName).textContent = "Загрузка"
    }
  ...
  function errorHandler(evt) {
    switch (evt.target.error.code) {
      ...
      case evt.target.error.NOT_READABLE_ERR:
        alert("Файл нечитабелен")
        break
      case evt.target.error.ABORT_ERR:
        break // noop
      default:
        alert("Произошла ошибка при чтении этого файла.")
    }
  }
}
